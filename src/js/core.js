...
function jackpot() {
  $('#jackpot-total').load('/core/jackpot/');
}
...
$(document).ready(function() {
  ...
  $('.bottom-info-block-opener').on('keyup click', function() {
    $('html, body').animate({ scrollTop: 0 }, 600);
  });
  $('.forget, .history').on('keyup click', function() {
    $('#pop-up, #pop-up2').hide();
    $('#pop-up3').show();
  });
  $('.login, .kassa').on('keyup click', function() {
    $('#pop-up2, #pop-up3').hide();
    $('#pop-up').show();
  });
  $('.close-btn').on('keyup click', function() {
    $('#pop-up, #pop-up2, #pop-up3').hide();
  });
  $('#or-register, .profil, .signup').on('keyup click', function() {
    $('#pop-up, #pop-up3').hide();
    $('#pop-up2').show();
  });
  $('#form1sub').on('keyup click', function() {
    $('#form1').submit();
  });
  $('#form2sub').on('keyup click', function() {
    $('#form2').submit();
  });
  $('#form3sub').on('keyup click', function() {
    $('#form3').submit();
  });
  ...
  $('.scrollup').on('keyup click', function() {
    $('html, body').animate({ scrollTop: 0 }, 600);
    return false;
  });
  $('.bitcoinbutton').on('keyup click', function() {
    $('#depositform').hide();
    $('#bitcoin').show();
  });
  $('.depway img').on('keyup click', function() {
    depway = $(this).attr('data-depositway');
    $('#depositway').attr('value', depway);
  });
  $('.depway').on('keyup click', function() {
    $('#phonemobile').hide();
    $('#phoneqiwi').hide();
    $('.depway').removeClass('active');
    $(this).addClass('active');
    document.getElementById('phoneqiwi').value = '';
    document.getElementById('phonemobile').value = '';
  });
  $('#phoneqiwi, #pr_pho, #phonemobile').on('change keyup input click', function() {
    if (this.value.match(/[^0-9]/g)) {
      this.value = this.value.replace(/[^0-9]/g, '');
    }
  });
  $('.depway.phonemobile').on('keyup click', function() {
    $('#phoneqiwi').hide();
    $('#phonemobile').show();
  });
  $('.depway.phoneqiwi').on('keyup click', function() {
    $('#phonemobile').hide();
    $('#phoneqiwi').show();
  });
  $('#depamount').on('keyup change input click', function() {
    mindep = Number(min);
    mindepbtc = Number(minbtc);
    depwaybtc = $('#depositway').val();
    if (depwaybtc == 'btc_usd') {
      mindep = mindepbtc;
    }
    if (depwaybtc == 'eth_usd') {
      mindep = mindepbtc;
    }
    ...
    amount = Number($('#depamount').val());
    if (mindep > amount) {
      $('#submitdeposit').attr('disabled', true).css({ 'opacity': '0.7' });
      $('#dep_error').html(deperr);
      return false;
    } else {
      $('#dep_error').html(deperr2);
  ...
  $('#submitdeposit').click(function() {
    ...
    if ((deway == 'qiwi_rub') || (deway == 'qiwi_usd') || (deway == 'qiwi_eur')) {
      if (phone.length < 11) {
        $('#dep_error').html(deperr3);
        return;
      }
    }
    if ((deway == 'mts_rub') || (deway == 'beeline_rub') || (deway == 'tele2_rub') || (deway == 'megafon_rub')) {
      if (phone.length < 10) {
        $('#dep_error').html(deperr4);
        return;
      }
    }
    if (phone == '') {
      $('.depositwindow').load('/deposit/pay/?payway=' + deway + '&amount=' + desum);
      ...
      if (errrr == 3) {
        $('#dep_error').html(deperr6);
      }
      if (errrr == 4) {
        $('#dep_error').html(deperr5);
      }
    } else {
  ...
  $('#set_nws').click(function() {
    if ($('#set_nws').is(':checked')) {
      $('#set_nws').val('1');
    } else {
      $('#set_nws').val('0');
    }
  });
  $('#set_bon').click(function() {
    if ($('#set_bon').is(':checked')) {
      $('#set_bon').val('1');
    } else {
      $('#set_bon').val('0');
    }
  ...
  $('.activatebutton3').on('keyup click', function() {
    phone();
  });
  ...
  $('.cancelbut2').on('keyup click', function() {
    cancel(2);
  });


  $('.outway img').on('keyup click', function() {
    outway = $(this).attr('data-payoutway');
    $('#wit_sys').attr('value', outway);
  });
  $('.outway').on('keyup click', function() {
    $('.outway').removeClass('active');
    $(this).addClass('active');
  });
  ...
  $('.hamburger').on('keyup click', function() {
    var $this = $(this);
    var backgroundImage = '';
  ...
  $('.searchlink').on('keyup click', function() {
  ...
  $(function() {
    $('#gamesearch').on('change keyup input click',
      function() {
        if (this.value.length >= 2) {
          $.ajax({
            type: 'post', url: '/core/search/',
            data: { 'gamesearch': this.value },
            cache: false,
